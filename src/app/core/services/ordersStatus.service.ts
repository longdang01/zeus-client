...
import { OrdersStatus } from '../models/OrdersStatus';
...
export class OrdersStatusService {
  API_URL: string = 'http://localhost:5000/api/ordersStatus';
  ...
  // create
  create(data: OrdersStatus): Observable<any> {
    const apiUrl = `${this.API_URL}/`;
    return this.httpClient.post(apiUrl, data, this.httpOptions)
  }
  
  // update
  update(id: any, data: OrdersStatus): Observable<any> {
    const apiUrl = `${this.API_URL}/${id}`;
